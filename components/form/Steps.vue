<template>
  <ul class="split-evenly w-full inline-flex flex-row gap-x-4">
    <li v-for="(step, index) in steps" :key="index" @click="handleClick(step)" :class="{
    ...
    }" class="flex-grow pt-2 px-1 pb-1 border-t-4 rounded-b hover:bg-white dark:hover:bg-gray-900 cursor-pointer">
